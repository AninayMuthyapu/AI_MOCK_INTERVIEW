...
/**
 * Login/Logout button with Google OAuth.
...
export default function LoginButton({ className = "", showUserInfo = true }: LoginButtonProps) {
    ...
    if (isLoading) {
        return (
            <button
                disabled
                className={`flex items-center gap-2 px-4 py-2 rounded-lg bg-white/10 text-white/60 ${className}`}
            >
    ...
    return (
        <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg bg-white hover:bg-gray-100 text-gray-800 transition-colors shadow-lg ${className}`}
        >
            {/* Google Logo */}
